...
const ACTUAL_USER = $('#actual_user').val()
...
socket.onmessage = async function(e){
    ...
    newMessage(message, sent_by_id, thread_id, sender_name)
}
...
function newMessage(message, sent_by_id, thread_id, sender_name) {
	if ($.trim(message) === '') {
		return false;
	}
	let message_element;
	let chat_id = 'chat_' + thread_id
	if(sent_by_id == ACTUAL_USER){
	    message_element = `
